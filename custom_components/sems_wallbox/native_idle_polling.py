"""Shared optional idle storage polling, isolated from control availability."""
...
class NativeIdlePolling:
    """Poll only while its diagnostic entity is enabled and TCP is confirmed."""

    IDLE_ONLY = True
    INTERVAL = 300
    TASK_NAME = "GoodWe idle storage"
    READ_METHOD = "async_read_energy"
    ...
    def available(self):
        ...
        return (
            ...
            and owner.local
            and not owner.transitioning
            and not owner._closed
            and owner.last_update_success
            and owner.transport.available
    ...
    async def tick(self):
        """Read at the configured interval, or once after a session ends."""
        owner = self.owner
        ...
        if (
            ...
            return
        idle = (
            transport.latest.stopped
            and transport.latest.state == 0
            and transport.session_guard.phase not in ("starting", "waiting", "charging")
        )
        if self.IDLE_ONLY and not idle:
            self.was_idle = False
            return
        ...
        try:
            value = await getattr(transport, self.READ_METHOD)()
